...
- [x] TTY rendering test approach checked.
...
- [x] MCP process test uses stdio pipes or SDK client transport.
...
- [x] Presenter tests cover TTY-only output behavior.
- [x] MCP stdio tests are isolated from CLI process and presenter tests.
...
- [x] Remove native terminal harness dependency after confirming current TTY behavior is formatting-only.
...
- [x] Add graph traversal, deterministic hybrid ranking, test-linking, and relationship graph hardening before treating ranking as the only issue.
- [x] Add adversarial, cross-OSS, and holdout eval packs and harden generic graph/ranking behavior until the recorded hash and Jina matrix is green.
- [x] Add path-level app-flow ranking to close the Dub and Formbricks holdout scoreboard misses.
- [x] Add pre-standalone diagnostics, benchmark harness, skipped-file reporting, and MCP agent guidance.
- [x] Resolve the Ladybug concurrent-read benchmark failure for the intended CLI/MCP access model.
- [ ] Tune Jina batching and model-cache ergonomics for faster semantic indexing.
- [x] Add public-facing README, package metadata, and install/MCP setup documentation for the standalone repository.
- [ ] Finish npm release automation and final published-package install validation.
...
- Added `testing-strategy.md` and layer-by-layer validation gates for unit, contract, integration, CLI, TTY rendering, database, MCP, end-to-end, eval, and proof-of-concept smoke coverage.
...
- [x] Standalone-readiness risk: Ladybug concurrent-read behavior is now measured and reports `pass` for the intended bounded CLI/MCP reader model after concurrency hardening.
- [x] `git diff --check` passed in `local-tools/code-intel`.
- [x] `git diff --check` passed in `local-doc/code-intelligence-graph`.
...
## 2026-05-18 07:07 CDT: Concurrency And Locking Verification

- [x] Red phase passed: `npx vitest run tests/integration/benchmark.test.ts` failed because `ladybugLock.concurrentRead` was `fail` and benchmark lock diagnostics were missing.
- [x] Fixed same-process reader self-contention by serializing public `QueryEngine` operations and making `LadybugGraphStore.open()` single-flight.
- [x] Fixed reader-during-update contention by scoping the process lock to the concrete Ladybug `.lbug` database path instead of the whole index root.
- [x] Added runtime telemetry for serialized query operations, queue wait time, max queue depth, process-lock wait time, Ladybug open retries, and lock contention count.
- [x] Added benchmark fields for `concurrentRead`, `readerDuringUpdate`, `readerAfterPublish`, `retryCount`, `queueWaitMs`, `lockWaitMs`, and `failureClassification`.
- [x] Added synthetic concurrency coverage in `tests/integration/concurrency.test.ts` for parallel reads, reader-during-update, reader-after-publish, stale generation behavior, live lock timeout reporting, and stale lock recovery.
- [x] Expanded MCP stdio coverage so concurrent `find_symbol`, `semantic_search`, and `get_callers` tool calls succeed through one server process.
- [x] Focused verification passed: `npm run build && npx vitest run tests/integration/concurrency.test.ts tests/integration/benchmark.test.ts tests/mcp/server.test.ts` returned 3 files passed and 6 tests passed.
- [x] Eval diagnostics smoke passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --diagnostics --json` returned `blockingStatus: pass`, `qualityStatus: pass`, and required 33/33.
- [x] CLI benchmark smoke passed with hash: `node dist/cli/main.js benchmark --suite js-ts-general --embedding-provider hash --skip-mcp-latency --json` returned `concurrentRead: pass`, `readerDuringUpdate: pass`, `readerAfterPublish: pass`, `failureClassification: none`, `retryCount: 0`, `queueWaitMs: 50`, and `lockWaitMs: 0`.
- [x] CLI benchmark smoke passed with Jina: `node dist/cli/main.js benchmark --suite js-ts-general --embedding-provider jina --skip-mcp-latency --json` returned `concurrentRead: pass`, `readerDuringUpdate: pass`, `readerAfterPublish: pass`, `failureClassification: none`, `retryCount: 0`, `queueWaitMs: 59`, and `lockWaitMs: 1`.
- [x] Full package verification passed: `npm test` returned 26 files passed, 1 skipped; 106 tests passed, 1 skipped. The skip remains the PTY-only test in this local runtime.
- [x] `git diff --check` passed in `local-tools/code-intel`.
- [x] `git diff --check` passed in `local-doc/code-intelligence-graph`.
- [x] `.DS_Store` scan passed across `local-tools/code-intel` and `local-doc/code-intelligence-graph`.
- [x] `npm pack --dry-run` passed with 341 files and 259.2 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.

## 2026-05-18 07:22 CDT: Standalone Local Repository Move Verification

- [x] Pre-move git status was clean in `local-tools/code-intel` and `local-doc/code-intelligence-graph`.
- [x] Imported `local-doc/code-intelligence-graph` into the `code-intel` repo under `docs/` using `git subtree add`, preserving the feature-doc commit history in the standalone repo graph.
- [x] Updated current docs and README references so active locations point at `/Users/jordy/Documents/GitHub/code-intel/` or repo-relative paths.
- [x] Historical verification entries intentionally keep their original `local-tools/code-intel` and `local-doc/code-intelligence-graph` paths because those were true when the checks ran.
- [x] Moved the repo directory to `/Users/jordy/Documents/GitHub/code-intel` and removed the old `capsule-org/local-doc/code-intelligence-graph` source folder after confirming docs were imported.
- [x] `git diff --check` passed from `/Users/jordy/Documents/GitHub/code-intel`.
- [x] `.DS_Store` scan passed inside `/Users/jordy/Documents/GitHub/code-intel`.
- [x] Full package verification passed from the new path: `npm test` returned 26 files passed, 1 skipped; 106 tests passed, 1 skipped.
- [x] `npm pack --dry-run` passed from the new path with 341 files and 259.2 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.

## 2026-05-18 07:43 CDT: Standalone Agent Instructions Verification

- [x] Added repo-local `AGENTS.md` for code-intel.
- [x] Added `docs/agent-workflows/` reference docs for development workflow, TDD, repo verification, local documentation, coding standards, and pull requests.
- [x] Removed parent-workspace-specific operational rules from the copied workflow, including multistack, child-repo, infrastructure, deployment, mobile, and Para-specific PR gates.
- [x] Preserved code-intel-relevant practices: feature docs as source of truth, requirements-first development, TDD, eval quality gates, verification evidence, docs updates, dirty-worktree protection, and conventional commits.
- [x] `git diff --check -- AGENTS.md docs/agent-workflows docs/feature-design-notes.md docs/verification-checklist.md` passed.

## 2026-05-18 09:13 CDT: Current Docs Reconciliation Verification

- [x] Reviewed commits after `9c1406c feat: harden relationship graph` through `29fb731 docs: add standalone agent instructions`.
- [x] Updated current-state docs to reflect adversarial evals, cross-OSS packs, holdout generalization, path-level ranking, diagnostics, benchmarks, concurrency hardening, and the standalone repo move.
- [x] Preserved older failed or partial eval entries as chronological evidence rather than rewriting history.
- [x] Left existing unrelated dirty work in `src/vectors/embedding.ts` and `src/workspace/discovery.ts` untouched.
- [x] `git diff --check` passed from `/Users/jordy/Documents/GitHub/code-intel`.

## 2026-05-18 10:36 CDT: Type Precision Hardening Verification

- [x] Added required `js-ts-adversarial` type-precision fixtures and eval cases for AST facts, graph/evidence edges, and query behavior.
- [x] Red phase passed: focused adversarial integration tests failed before implementation because AST `typeReferences` were missing and graph edges lacked `tree-sitter-type-reference` evidence.
- [x] Red phase passed: old-dist hash eval against the updated pack failed the new required type-precision gates after the graph case schema was corrected.
- [x] Added persisted Tree-sitter `typeReferences` facts for type aliases, generic constraints/defaults/instantiations, mapped and conditional types, indexed access, `keyof`, `typeof`, and namespace-qualified type members.
- [x] Added graph integration for AST type-reference `REFERENCES` edges with `tree-sitter-type-reference` and `type-use` evidence, while preserving existing SCIP-backed type relationships.
- [x] Added query precision fixes for exact qualified enum-member seeds and anonymous default caller fallback compatibility.
- [x] Focused verification passed: `npm run build`.
- [x] Focused verification passed: `npx vitest run tests/integration/adversarial-eval.test.ts -t "type precision|type-alias dependency|generic constraint|generic instantiation|enum member references|merged namespace|qualified enum"` returned 8 passed and 30 skipped.
- [x] Full adversarial integration verification passed: `npx vitest run tests/integration/adversarial-eval.test.ts` returned 38 passed.
- [x] Synthetic adversarial hash eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned required 44/44, target 44/44, scoreboard 5/5, `blockingStatus: pass`, and `qualityStatus: pass`.
- [x] Synthetic general hash eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --json` returned required 33/33 and `blockingStatus: pass`.
- [x] Full package verification initially hit a timeout in `tests/integration/incremental-update.test.ts`; isolated rerun passed with 2 tests passed in 25.42s, classifying the first failure as timing-sensitive full-suite contention rather than an assertion failure.
- [x] Full package verification passed on rerun: `npm test` returned 26 files passed, 1 skipped; 115 tests passed, 1 skipped. The skip remains the PTY-only test in this local runtime.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed: `npm pack --dry-run` reported 351 files and 264.8 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.
- [x] Final synthetic adversarial hash eval passed after full verification: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned required 44/44, target 44/44, scoreboard 5/5, and 93/93 total.
- [x] Final synthetic general hash eval passed after full verification: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --json` returned required 33/33 and 33/33 total.
- [x] Final synthetic adversarial Jina eval passed after full verification: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider jina --json` returned required 44/44, target 44/44, scoreboard 5/5, and 93/93 total.
- [x] Final synthetic general Jina eval passed after full verification: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider jina --json` returned required 33/33 and 33/33 total.

## 2026-05-18 12:21 CDT: MCP Agent-Readiness Verification

- [x] Added required `js-ts-adversarial` MCP agent-readiness eval case covering SDK stdio workflow chaining, structured output schemas, diagnostics tools, relationship browsing, ranking reasons, relationship evidence, path edges, invalid input handling, and bounded context.
- [x] Red phase passed: `npm run build && npx vitest run tests/mcp/server.test.ts tests/unit/evaluator.test.ts` failed before implementation because `get_relationships`, `diagnose_file`, and `diagnose_symbol` were missing, MCP calls lacked `structuredContent`, and `mcpCaseFiles` were not loaded.
- [x] Added MCP `outputSchema` and `structuredContent` for every tool while preserving text content compatibility.
- [x] Added MCP `diagnose_file` and `diagnose_symbol` tools backed by generation-local diagnostics.
- [x] Added shared query-engine `getRelationships()` support and MCP `get_relationships` with direction, edge-kind filters, bounded limits, and relationship evidence metadata.
- [x] Added eval-pack `mcpCaseFiles` support and MCP workflow result reporting.
- [x] Focused verification passed: `npm run build && npx vitest run tests/mcp/server.test.ts tests/unit/evaluator.test.ts` returned 2 files passed and 7 tests passed.
- [x] Synthetic adversarial hash eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned required 45/45, target 44/44, scoreboard 5/5, and 94/94 total.
- [x] Synthetic adversarial Jina eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider jina --json` returned required 45/45, target 44/44, scoreboard 5/5, and 94/94 total.
- [x] Synthetic general hash eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --json` returned required 33/33 and 33/33 total.
- [x] Synthetic general Jina eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider jina --json` returned required 33/33 and 33/33 total.
- [x] Full package verification passed: `npm test` returned 26 files passed, 1 skipped; 116 tests passed, 1 skipped. The skip remains the PTY-only test in this local runtime.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed: `npm pack --dry-run` reported 358 files and 275.9 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.

## 2026-05-18 14:25 CDT: CLI API And CLI/MCP Parity Verification

- [x] Added CLI `relationships <seed>` backed by shared `QueryEngine.getRelationships()` with `--edge-kind`, `--direction`, and bounded `--limit` support.
- [x] Added TTY query-result summaries while preserving deterministic JSON for `--json` and non-TTY output.
- [x] Added required `js-ts-adversarial` CLI/MCP parity eval case covering semantic search, symbol lookup, relationship browsing, path tracing, diagnostics, invalid input, bounded context, stable IDs, files, symbols, ranking reasons, relationship evidence, path edges, limits, and no source dumps outside context tools.
- [x] Added eval-pack `cliMcpCaseFiles` support and CLI/MCP parity result reporting.
- [x] Added `docs/cli-reference.md` with command groups, command purpose, arguments, options, JSON output contracts, human output behavior, and realistic agent chains.
- [x] Red phase passed: `npm run build && npx vitest run tests/unit/cli-program.test.ts tests/unit/presenter.test.ts tests/cli/process.test.ts tests/integration/cli-mcp-parity.test.ts tests/unit/evaluator.test.ts` failed before implementation because `relationships` was not registered, TTY query output still rendered raw JSON, CLI process help/query calls could not find `relationships`, and `cliMcpCaseFiles` were not loaded.
- [x] Focused verification passed: `npm run build && npx vitest run tests/unit/cli-program.test.ts tests/unit/presenter.test.ts tests/cli/process.test.ts tests/integration/cli-mcp-parity.test.ts tests/unit/evaluator.test.ts tests/mcp/server.test.ts` returned 6 files passed and 26 tests passed.
- [x] Synthetic adversarial hash eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned required 46/46, target 44/44, scoreboard 5/5, and 95/95 total.
- [x] Synthetic adversarial Jina eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider jina --json` returned required 46/46, target 44/44, scoreboard 5/5, and 95/95 total.
- [x] Synthetic general hash eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --json` returned required 33/33 and 33/33 total.
- [x] Synthetic general Jina eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider jina --json` returned required 33/33 and 33/33 total.
- [x] Full package verification initially hit a single transient `tests/integration/scip.test.ts` `run.ok` false result; isolated rerun passed immediately, classifying the first failure as timing-sensitive SCIP process setup rather than a product assertion failure.
- [x] Full package verification passed on rerun: `npm test` returned 27 files passed, 1 skipped; 123 tests passed, 1 skipped. The skip remains the PTY-only test in this local runtime.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed: `npm pack --dry-run` reported 362 files and 285.9 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.

## 2026-05-18 16:01 CDT: Native Terminal Harness Removal Verification

- [x] Removed the `node-pty` dev dependency from `package.json` and `package-lock.json`.
- [x] Removed the `test:pty` script and deleted the skipped `tests/pty/health.test.ts` test.
- [x] Updated repo workflow docs so current TTY rendering coverage lives in presenter/unit tests, while CLI process behavior remains covered by `execa` pipes and MCP remains covered by SDK stdio transport.
- [x] Rationale: current CLI TTY behavior is formatting-only; there is no raw terminal input, resize handling, progress UI, or color contract that requires native pseudo-terminal allocation.
- [x] Dependency check passed: `npm ls node-pty` returned an empty dependency tree.
- [x] Build verification passed: `npm run build`.
- [x] Full package verification passed without skips: `npm test` returned 27 files passed and 123 tests passed.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed: `npm pack --dry-run` reported 362 files and 285.9 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.

## 2026-05-18 16:38 CDT: Public Repository Metadata Verification

- [x] Replaced the local-development README with a public-facing README covering purpose, install from source, tarball install, quick start, MCP configuration, command surface, embeddings, evals, benchmarks, and development verification.
- [x] Updated package metadata with the `jlm0/code-intel` repository URL, issue URL, homepage, keywords, public package description, and `prepack` build hook.
- [x] Removed the package-level private publish guard so the package can be packed or published when the release process is ready.
- [x] Kept licensing unchanged. Public repository visibility does not add an open-source license by itself.
- [x] Lockfile refresh passed: `npm install --package-lock-only --ignore-scripts`.
- [x] Build verification passed: `npm run build`.
- [x] Initial `npm test` run hit one transient adversarial eval aggregate failure after 122 tests passed; the focused rerun of `tests/integration/adversarial-eval.test.ts -t "synthetic adversarial pack still passes every gate end-to-end"` passed immediately.
- [x] Full package verification passed on rerun: `npm test` returned 27 files passed and 123 tests passed.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the new `prepack` build hook: `npm pack --dry-run` reported 362 files and 285.0 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.
- [x] GitHub repository created at `https://github.com/jlm0/code-intel` with public visibility.
- [x] Repository description, homepage, and topics were set through `gh repo edit`.

## 2026-05-18 18:30 CDT: Versioned Local Release Verification

- [x] Added `release:local` and `version:bump` scripts for controlled local release and later version bumps.
- [x] Added local release scripts that pack into `.local-releases/v<version>/`, install the exact tarball globally, verify the installed CLI version, and write `manifest.json`.
- [x] Added CLI version tests proving `code-intel --version` uses the package version from `package.json`.
- [x] Added `.local-releases/` to `.gitignore` so local tarballs and manifests do not enter public history.
- [x] Focused verification passed: `npm run build`.
- [x] Focused verification passed: `npx vitest run tests/unit/cli-program.test.ts tests/cli/process.test.ts` returned 2 files passed and 17 tests passed.
- [x] Script help verification passed: `node scripts/bump-version.mjs --help` and `node scripts/local-release.mjs --help`.
- [x] Full package verification initially hit timeout-only failures in `tests/integration/cli-mcp-parity.test.ts` and `tests/integration/incremental-update.test.ts`; isolated reruns passed, classifying them as full-suite runtime headroom issues rather than assertion failures.
- [x] Long-running integration tests now have 120 second per-test timeout headroom without changing assertions.
- [x] Full package verification passed: `npm test` returned 27 files passed and 125 tests passed.
- [x] Local release script passed: `npm run release:local` ran tests, packed `code-intel-0.1.0.tgz`, installed it globally, and verified the installed version.
- [x] Installed CLI verification passed: `which code-intel` returned `/Users/jordy/.nvm/versions/node/v22.17.0/bin/code-intel`, `code-intel --version` returned `0.1.0`, and `npm list -g code-intel --depth=0` returned `code-intel@0.1.0`.
- [x] Diff whitespace verification passed: `git diff --check`.

## 2026-05-19 20:05 CDT: Index Progress Verification

- [x] Added shared progress schema and core progress utilities for `<indexPath>/progress/current.json`.
- [x] Added progress wiring for `indexWorkspace()` and `updateWorkspace()` with coarse phases, failed-state reporting, and final succeeded state after publish.
- [x] Added CLI `progress`, `status.progress`, stderr-only live progress for `index` and `update`, and `--quiet` suppression.
- [x] Added MCP `index_progress` plus `workspace_overview` progress visibility and guidance metadata.
- [x] Red phase passed: `npx vitest run tests/unit/progress.test.ts tests/unit/cli-program.test.ts tests/integration/progress.test.ts tests/cli/process.test.ts tests/mcp/server.test.ts` failed before implementation because `src/core/progress.js`, `progress`, `index_progress`, and stderr progress output were missing.
- [x] Focused verification passed: `npx vitest run tests/unit/progress.test.ts tests/unit/cli-program.test.ts tests/integration/progress.test.ts tests/cli/process.test.ts tests/mcp/server.test.ts` returned 5 files passed and 27 tests passed.
- [x] Full package verification initially hit a timeout in the existing concurrent CLI read process test after the process file gained more fixture indexing work; `npm run test:cli` passed after giving that long-running test a 60 second timeout.
- [x] Full package verification passed on final rerun: `npm test` returned 29 files passed and 134 tests passed.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the `prepack` build hook: `npm pack --dry-run` reported 365 files and 290.1 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.

## 2026-05-20 09:12 CDT: Substep Progress And JSONL Event Verification

- [x] Added red tests for progress JSONL logs, recent event querying, write-lock state, rich failure details, SCIP quality warnings, discovery summaries, graph substeps, CLI `--events`, and MCP `includeEvents`.
- [x] Red phase passed: `npx vitest run tests/unit/progress.test.ts tests/unit/scip-quality.test.ts tests/unit/cli-program.test.ts tests/integration/progress.test.ts tests/cli/process.test.ts tests/mcp/server.test.ts` failed before implementation because `progressLogPath`, `src/scip/quality.js`, `--events`, recent events, write-lock output, rich error details, and substep events were missing.
- [x] Focused unit verification passed: `npx vitest run tests/unit/progress.test.ts tests/unit/scip-quality.test.ts tests/unit/cli-program.test.ts` returned 3 files passed and 17 tests passed.
- [x] Focused integration verification passed: `npx vitest run tests/integration/progress.test.ts` returned 1 file passed and 2 tests passed.
- [x] Focused CLI/MCP verification passed: `npx vitest run tests/cli/process.test.ts tests/mcp/server.test.ts` returned 2 files passed and 14 tests passed.
- [x] Manual output review passed after indexing `tests/fixtures/js-ts-workspace` with hash embeddings: `progress --events --limit 50 --json` returned `status: succeeded`, `writeLock.status: unlocked`, 27 recent events, graph substeps, a `scip_quality` event with 53 definitions, 63 references, 131 occurrences, 33,605 output bytes, and a `discovery_summary` with 29 included files, 3 packages, and zero unsupported, ignored, tsconfig-excluded, or outside-source-root files.
- [x] Focused verification passed after output cleanup: `npx vitest run tests/unit/progress.test.ts tests/unit/scip-quality.test.ts tests/unit/cli-program.test.ts tests/integration/progress.test.ts tests/cli/process.test.ts tests/mcp/server.test.ts` returned 6 files passed and 33 tests passed.
- [x] Full package verification passed: `npm test` returned 30 files passed and 139 tests passed.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the `prepack` build hook: `npm pack --dry-run` reported 371 files and 298.0 kB package size. No `code-intel-0.1.0.tgz` artifact was left behind.

## 2026-05-20 11:02 CDT: Indexing Performance Correction Verification

- [x] Added focused regressions for all five correction points: SCIP inferred config does not write source `tsconfig.json`, no-op update remains incremental without source `tsconfig.json`, `tsconfig.base.json` edits invalidate config, duplicate missing embedding inputs are embedded once, and existing graph/query/adversarial tests guard typed projection removal plus test-linking traversal behavior.
- [x] Red phase passed: `npx vitest run tests/integration/scip.test.ts tests/integration/incremental-update.test.ts tests/unit/fingerprints.test.ts tests/unit/chunk-embeddings.test.ts` failed before implementation for the expected five reasons: source `tsconfig.json` was written, no-op update saw the generated config, `tsconfig.base.json` did not affect config hash, duplicate embedding input was embedded twice, and `tsconfig.base.json` update stayed incremental.
- [x] Focused correction tests passed: `npx vitest run tests/integration/scip.test.ts tests/integration/incremental-update.test.ts tests/unit/fingerprints.test.ts tests/unit/chunk-embeddings.test.ts` returned 4 files passed and 8 tests passed.
- [x] Build verification passed: `npm run build`.
- [x] Focused graph/query/benchmark harness verification passed: `npx vitest run tests/integration/adversarial-eval.test.ts tests/integration/index-query.test.ts tests/integration/benchmark.test.ts` returned 3 files passed and 41 tests passed.
- [x] Full package tests passed: `npm test` returned 32 files passed and 145 tests passed.
- [x] Synthetic general hash eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, and required 33/33.
- [x] Synthetic adversarial hash eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, required 46/46, target 44/44, scoreboard 5/5, and 95/95 total.
- [x] Synthetic general Jina eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider jina --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, and required 33/33.
- [x] Synthetic adversarial Jina eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider jina --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, required 46/46, target 44/44, scoreboard 5/5, and 95/95 total.
- [x] Rallly hash benchmark passed: `node dist/cli/main.js benchmark --suite oss-rallly-app-flow --fetch --eval-cache-path /tmp/code-intel-rallly-benchmark-cache --embedding-provider hash --skip-mcp-latency --json` returned warm update `mode: incremental`, files added 0, changed 0, deleted 0, unchanged 114, chunks embedded 0, reused 403, and no config-change rebuild.
- [x] Rallly adversarial hash eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/oss-rallly-adversarial --eval-cache-path /tmp/code-intel-rallly-eval-cache --embedding-provider hash --fetch --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, and target 17/17.
- [x] Synthetic hash benchmark passed: `node dist/cli/main.js benchmark --suite js-ts-general --embedding-provider hash --skip-mcp-latency --json` returned warm update `mode: incremental`, files unchanged 29, chunks embedded 0, reused 43, and Ladybug lock checks passed.
- [x] Synthetic Jina benchmark passed: `node dist/cli/main.js benchmark --suite js-ts-general --embedding-provider jina --skip-mcp-latency --json` returned warm update `mode: incremental`, files unchanged 29, chunks embedded 0, reused 43, and Ladybug lock checks passed.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the `prepack` build hook: `npm pack --dry-run` reported 371 files and 301.5 kB package size. No `code-intel-*.tgz` artifact was left behind.

## 2026-05-20 12:04 CDT: MCP Server Version Metadata Verification

- [x] Added an MCP regression assertion that the initialized server metadata reports `name: code-intel` and the current `package.json` version.
- [x] Red phase passed: `npm run test:mcp` failed before implementation because MCP `serverInfo.version` reported `0.1.0` while `package.json` reported `0.2.0`.
- [x] Shared the package-version reader between CLI and MCP server setup so both surfaces report the installed package version.
- [x] Focused MCP verification passed: `npm run test:mcp` returned 1 file passed and 2 tests passed.
- [x] Focused CLI version verification passed: `npx vitest run tests/unit/cli-program.test.ts` returned 1 file passed and 7 tests passed.
- [x] Final TypeScript build verification passed with direct compiler invocation: `node node_modules/typescript/bin/tsc -p tsconfig.json`. The equivalent `npm run build` command hung in this Codex shell while launching the `.bin/tsc` shebang, matching the local script-launch issue observed with direct executable scripts.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the `prepack` build hook: `npm pack --dry-run` reported 374 files and 301.6 kB package size. No `code-intel-*.tgz` artifact was left behind.
- [x] Installed MCP metadata verification passed by launching the installed CLI entrypoint with explicit Node and an SDK stdio client; `client.getServerVersion()` returned `{"name":"code-intel","version":"0.2.0"}`.
- [x] Local reinstall caveat: `npm run release:local -- --skip-tests` packed and wrote the fixed installed package files but hung in global npm postinstall around copied `tree-sitter-typescript` native binding loading. The local working dependency loads cleanly, so the global package was repaired with a symlink to the local `tree-sitter-typescript` dependency and a wrapper bin script for this machine.

## 2026-05-21 11:19 CDT: Capsule Workspace OOM Red Phase

- [x] Added RED guard tests for the confirmed and partially confirmed OOM findings in `tests/unit/indexing-oom-regressions.test.ts` and `tests/unit/indexing-memory-contracts.test.ts`.
- [x] Focused RED verification passed: `npx vitest run tests/unit/indexing-oom-regressions.test.ts tests/unit/indexing-memory-contracts.test.ts` failed intentionally with 18 expected contract failures covering relationship-graph repo scoping, transitive call dedupe, cross-repo import lookup, duplicate call promotion, stale progress semantics, step heartbeat events, stale write-lock freshness, SCIP sharding, SCIP heap/cache controls, SCIP failure reporting, tiny SCIP fallback, per-repo SCIP spilling, bounded file fact IO, incremental embeddings, streaming Ladybug writes, streaming JSON artifact writes, and shared EPERM-aware liveness.
- [x] Whitespace verification passed for the new RED tests: `awk '/[ \t]$/ { print FILENAME ":" FNR ": trailing whitespace"; bad=1 } END { exit bad }' tests/unit/indexing-oom-regressions.test.ts tests/unit/indexing-memory-contracts.test.ts docs/verification-checklist.md`.
- [x] Green phase was deferred to the follow-up OOM hardening pass.

## 2026-05-21 12:15 CDT: Capsule Workspace OOM Green Phase

- [x] Implemented the 18 OOM and memory-contract corrections guarded by `tests/unit/indexing-oom-regressions.test.ts` and `tests/unit/indexing-memory-contracts.test.ts`.
- [x] Relationship graph corrections scope imported-member lookup and transitive call expansion to current-repo evidence, dedupe direct call edges before transitive expansion, and leave call-evidence promotion to the final promotion step instead of re-running it during relationship graphing.
- [x] Progress and lock corrections keep live CPU-bound graph steps running when the writer PID is alive, add `step_progress` as a durable event type, treat very old live-PID write locks as stale without fresh owner evidence, and make index and Ladybug lock liveness EPERM-aware.
- [x] SCIP corrections add explicit `projectPaths`, run `scip-typescript` through Node with `--max-old-space-size`, pass `--no-global-caches`, avoid `step_succeeded` for failed child processes, and route `scip-empty-or-tiny` successful output through Tree-sitter fallback.
- [x] Memory corrections spill per-repo SCIP facts to disk until publish, keep diagnostics on per-file SCIP counts instead of retained facts, fingerprint files with bounded concurrency, embed missing chunk inputs incrementally, pass graph maps into Ladybug without rebuild array copies, and stream JSON artifact writes.
- [x] First full-suite rerun exposed a real eval regression: package-sharded inferred SCIP configs lost root path aliases. The fix keeps package sharding but makes inferred shard configs extend the repo root `tsconfig.json` or `jsconfig.json`, preserving compiler resolution without reintroducing a root-wide SCIP project.
- [x] Focused green verification passed: `npx vitest run tests/unit/indexing-oom-regressions.test.ts tests/unit/indexing-memory-contracts.test.ts` returned 2 files passed and 18 tests passed.
- [x] Build verification passed: `npm run build`.
- [x] Nearby unit verification passed: `npx vitest run tests/unit/indexing-oom-regressions.test.ts tests/unit/indexing-memory-contracts.test.ts tests/unit/progress.test.ts tests/unit/scip-quality.test.ts tests/unit/chunk-embeddings.test.ts` returned 5 files passed and 30 tests passed.
- [x] SCIP, graph, progress, and diagnostics integration verification passed: `npx vitest run tests/integration/scip.test.ts tests/integration/index-query.test.ts tests/integration/fusion-resolution.test.ts tests/integration/progress.test.ts tests/integration/diagnostics.test.ts` returned 5 files passed and 9 tests passed.
- [x] Full package tests passed: `npm test` returned 34 files passed and 163 tests passed.
- [x] Synthetic general hash eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, and required 33/33.
- [x] Synthetic adversarial hash eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, required 46/46, target 44/44, scoreboard 5/5, and 95/95 total.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the `prepack` build hook: `npm pack --dry-run` reported 374 files and 305.5 kB package size. No `code-intel-*.tgz` artifact was left behind.

## 2026-05-21 15:58 CDT: Sharded SCIP Red Phase

- [x] Added RED guards in `tests/integration/scip-sharding.test.ts` for three behavior contracts: separately produced package shard outputs must preserve cross-package references, repos with workspace packages must still SCIP-index discovered root-level test/script files, and inferred package shard configs must not surface generated files from discovery-ignored directories.
- [x] Added RED guards in `tests/unit/scip-sharding-contracts.test.ts` for five memory and artifact contracts: do not batch multiple project shards into one `scip-typescript` child, keep default SCIP heap at or below 1 GB, write raw SCIP artifacts per shard, spill facts by shard identity, and align inferred shard excludes with discovery ignored generated directories.
- [x] Focused RED verification passed: `npx vitest run tests/unit/scip-sharding-contracts.test.ts tests/integration/scip-sharding.test.ts` failed intentionally with 8 expected failures covering one-process/many-project execution, 6 GB heap default, repo-wide `.scip` output, repo-only fact spilling, missing generated-directory inferred excludes, cross-shard reference loss for `packages/ui/src/useGivingSummary.tsx`, missing root-level `e2e/root-flow.test.ts` SCIP coverage, and generated `packages/app/generated/schema.ts` leakage.
- [x] Whitespace verification passed for the new RED tests and updated feature docs: `awk '/[ \t]$/ { print FILENAME ":" FNR ": trailing whitespace"; bad=1 } END { exit bad }' tests/integration/scip-sharding.test.ts tests/unit/scip-sharding-contracts.test.ts docs/feature-spec.md docs/feature-plan.md docs/testing-strategy.md docs/feature-design-notes.md docs/verification-checklist.md`.
- [x] Green phase is intentionally deferred to the next implementation pass.

## 2026-05-21 16:41 CDT: Sharded SCIP Green Phase

- [x] Implemented sharded SCIP planning from code-intel discovery: package shards constrained to discovered package files, a repo-level shard for discovered files outside packages, and a fallback repo shard when no package/source-root shard exists.
- [x] Enforced one `scip-typescript` child per shard, repo-root `cwd`, disabled global caches, default heap capped at 1 GB, and raw SCIP outputs keyed by `repo/shard`.
- [x] Added inferred shard config constraints for explicit shard file lists and generated-directory excludes, while preserving package or root `tsconfig.json` / `jsconfig.json` extension for compiler options and aliases.
- [x] Added merge-aware SCIP ingestion so references from one shard can resolve against definitions emitted by another shard before graph fusion.
- [x] Added shard-keyed scratch fact spills and final repo-level fact merging so the published `facts/scip.json` remains one logical entry per repo.
- [x] Added a package-config regression guard after the green pass exposed a narrower gap: broad package `tsconfig.json` includes can no longer reintroduce generated files ignored by discovery.
- [x] Focused green verification passed: `npx vitest run tests/unit/scip-sharding-contracts.test.ts tests/integration/scip-sharding.test.ts` returned 2 files passed and 9 tests passed.
- [x] Build verification passed: `npm run build`.
- [x] Nearby sharding, SCIP, graph, progress, diagnostics, and memory-contract verification passed: `npx vitest run tests/unit/scip-sharding-contracts.test.ts tests/unit/indexing-memory-contracts.test.ts tests/unit/indexing-oom-regressions.test.ts tests/unit/scip-quality.test.ts tests/integration/scip-sharding.test.ts tests/integration/scip.test.ts tests/integration/index-query.test.ts tests/integration/fusion-resolution.test.ts tests/integration/progress.test.ts tests/integration/diagnostics.test.ts` returned 10 files passed and 38 tests passed.
- [x] Full package tests initially timed out in `tests/integration/benchmark.test.ts` and `tests/cli/process.test.ts` under the default high Vitest worker count; both isolated reruns passed, classifying the failures as local process-heavy suite contention rather than assertion failures.
- [x] Test harness concurrency is now bounded with `maxWorkers: 4`; full package verification passed: `npm test` returned 36 files passed and 172 tests passed.
- [x] Synthetic general hash eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider hash --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, and required 33/33.
- [x] Synthetic adversarial hash eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, required 46/46, target 44/44, scoreboard 5/5, and 95/95 total.
- [x] Synthetic hash benchmark passed: `node dist/cli/main.js benchmark --suite js-ts-general --embedding-provider hash --skip-mcp-latency --json` returned warm update `mode: incremental`, one-file update `changed: 1`, deleted-file update `deleted: 1`, query latency `semanticMs: 244`, Ladybug lock checks passed, and peak RSS 712 MB.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the `prepack` build hook: `npm pack --dry-run` reported 374 files and 308.0 kB package size. No `code-intel-*.tgz` artifact was left behind.

## 2026-05-21 18:42 CDT: Local 0.3.0 Release

- [x] Version bump applied with `npm run version:bump -- 0.3.0`, updating `package.json` and `package-lock.json`.
- [x] Local release tests passed through `npm run release:local`: `npm test` returned 36 files passed and 172 tests passed.
- [x] Local release pack passed through `npm run release:local`: `npm pack --pack-destination .local-releases/v0.3.0` produced `code-intel-0.3.0.tgz` with 374 files and 308.0 kB package size.
- [x] Initial global install failed with npm `ENOTEMPTY` because a stale `.code-intel-*` rename directory from a previous failed install was still present under the global node_modules path.
- [x] Removed the stale `.code-intel-*` global npm temporary directory and installed `.local-releases/v0.3.0/code-intel-0.3.0.tgz` with `npm install -g`.
- [x] Installed package verification passed: `npm list -g code-intel --depth=0` reports `code-intel@0.3.0`.
- [x] Installed CLI verification passed with explicit Node launch: `node /Users/jordy/.nvm/versions/node/v22.17.0/lib/node_modules/code-intel/dist/cli/main.js --version` returned `0.3.0`.
- [x] Installed MCP metadata verification passed by launching the installed CLI entrypoint with explicit Node and an SDK stdio client; `client.getServerVersion()` returned `{"name":"code-intel","version":"0.3.0"}`.

## 2026-05-21 20:43 CDT: Embedding Batch Progress Verification

- [x] Added RED guard coverage that `embedGraphChunks` reports batch start and completion telemetry with batch index, batch size, completed batch count, embedded chunk count, and visited chunk count.
- [x] Added integration coverage that index progress events expose `currentStep: embedding-batch` with `step_progress`, embedding counters, and memory telemetry through the public progress event stream.
- [x] RED verification passed: `npx vitest run tests/unit/chunk-embeddings.test.ts` failed before implementation because no batch progress callbacks were emitted.
- [x] RED integration verification passed: `npx vitest run tests/integration/progress.test.ts` failed before implementation because no `embedding-batch` progress event reached the durable event log.
- [x] Build verification passed: `npm run build`.
- [x] Focused progress verification passed: `npx vitest run tests/unit/chunk-embeddings.test.ts tests/integration/progress.test.ts` returned 2 files passed and 4 tests passed.
- [x] Nearby schema/progress verification passed: `npx vitest run tests/unit/progress.test.ts tests/unit/schema.test.ts` returned 2 files passed and 13 tests passed.
- [x] Full package tests passed: `npm test` returned 36 files passed and 173 tests passed.
- [x] Diff whitespace verification passed: `git diff --check`.
- [x] Package dry-run passed through the `prepack` build hook: `npm pack --dry-run` reported 374 files and 309.1 kB package size. No `code-intel-*.tgz` artifact was left behind.

## 2026-05-21 22:04 CDT: Jina And Tree-sitter Embedding Input Hardening Checkpoints

Reference label: `workstream:jina-tree-sitter-embedding-input-hardening`

This section is the checkpoint contract for the next agentic implementation sessions. Do not mark an item complete or start the next item until its focused tests or evals pass, relevant docs are updated, and any large-workspace observation is recorded here.

Item 1: Jina token telemetry

- [x] Unit coverage proves token-stat aggregation, oversized classification, reusable embedding input counts, and report shape without requiring a model download.
- [x] Integration or benchmark coverage proves public telemetry includes token buckets, max tokens, oversized count, batch max tokens, and padding-waste indicators.
- [x] A local workspace observation records which files, paths, or chunk kinds create the longest Jina inputs.
- [x] Existing hash and Jina eval behavior is unchanged except for added telemetry fields and the new target gate.

Item 2: generated, build, cache, and hidden artifact exclusion

- [x] Unit and integration coverage prove artifact paths produce no chunks, no SCIP facts, no exact-search results, no graph nodes, and no embeddings.
- [x] Coverage proves an explicitly allowlisted hidden source-like path can still be included.
- [x] Diagnostics explain skipped artifact paths with stable reasons.
- [x] A local workspace diagnostics pass shows artifact directories no longer dominate included file, chunk, or embedding counts.
- [x] Existing source, graph, query, and eval gates remain green.

Item 3: length-aware embedding batching

- [x] Unit coverage proves batching groups similar token lengths, preserves deterministic output assignment, and handles duplicate reusable inputs.
- [x] Focused benchmark or integration coverage shows lower padded-token cost, wall-clock time, or peak memory on a representative corpus.
- [x] Jina eval results remain equivalent within documented floating-point and ranking tolerance.
- [x] No ranking rules or provider behavior changes are made in this item.

Item 4: truncation-loss eval targets

- [x] New target evals place relevant raw-source evidence after the old blind character-truncation point.
- [x] New false-positive guards prove generated or built artifacts do not outrank raw source.
- [x] Red verification proves the current implementation fails these cases for discovery, chunking, embedding, query, or ranking reasons that are visible in the report.
- [x] Existing required gates are not weakened, deleted, or reclassified to make the red phase cleaner.

Item 5: token-aware Tree-sitter splitting for oversized source

- [x] Unit and integration coverage prove oversized real source becomes multiple bounded chunks with stable ranges and no silent truncation.
- [x] Child chunks preserve parent context, symbol metadata, file range, source hash, and source/context provenance.
- [x] Item 4 eval targets pass under Jina and avoid generated-artifact false positives.
- [x] Hash-backed required gates, Jina-backed semantic gates, and affected adversarial evals remain green.
- [x] Benchmark output records token counts, split counts, truncation fallback counts, embedding runtime, and peak memory.

Implementation evidence:

- [x] Red phase: `npx vitest run tests/unit/chunk-embeddings.test.ts tests/unit/embedding-input.test.ts tests/integration/discovery.test.ts tests/integration/exact-search.test.ts tests/integration/progress.test.ts` failed before implementation for the expected reasons: hidden directories were included, exact search found `.vercel`, embedding progress lacked token counters, count-based batching preserved mixed lengths, and `chunkEmbeddingInput` silently truncated late source.
- [x] Red phase: old-dist hash adversarial eval failed the new `adv.target.embedding.after-truncation-signal` target because `.vercel/output/oversized-generated.ts` appeared in the top results.
- [x] Focused green tests passed: `npx vitest run tests/unit/chunk-embeddings.test.ts tests/unit/embedding-input.test.ts tests/integration/discovery.test.ts tests/integration/exact-search.test.ts tests/integration/progress.test.ts tests/integration/embedding-input-hardening.test.ts` returned 6 files and 24 tests passed.
- [x] Full test suite passed after the final 512-token target-budget change: `npm test` returned 38 files and 181 tests passed.
- [x] Jina general eval passed: `node dist/cli/main.js eval --suite js-ts-general --embedding-provider jina --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, required 33/33, and embedding input telemetry with token budget 512, max 78, p95 52, and truncation fallbacks 0.
- [x] Jina adversarial eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider jina --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, required 46/46, target 45/45, scoreboard 5/5, and the new after-truncation target ranked `packages/syntax/src/oversized-embedding-source.ts` at rank 1 while `.vercel/output/oversized-generated.ts` was absent.
- [x] Hash adversarial eval passed: `node dist/cli/main.js eval --eval-pack eval-packs/js-ts-adversarial --embedding-provider hash --json` returned `status: pass`, `blockingStatus: pass`, `qualityStatus: pass`, required 46/46, target 45/45, scoreboard 5/5, and the new after-truncation target ranked the raw source at rank 1 while the generated decoy was absent.
- [x] Hash benchmark smoke passed: `node dist/cli/main.js benchmark --suite js-ts-general --embedding-provider hash --skip-mcp-latency --json` returned 43 chunks, cold index 4851 ms, warm update reused 43 chunks, peak RSS 402 MB, token budget 512, max input tokens 36, and truncation fallbacks 0.
- [x] Jina benchmark smoke passed: `node dist/cli/main.js benchmark --suite js-ts-general --embedding-provider jina --skip-mcp-latency --json` returned 43 chunks, cold index 30180 ms, warm update reused 43 chunks, peak RSS 1010 MB, token budget 512, max input tokens 78, and truncation fallbacks 0.
- [x] Local self-index Jina observation passed from `/tmp/code-intel-self-jina-hardening`: 194 files, 1,332 chunks, 1,268 unique embedding inputs, 190 split chunks, token budget 512, max 512, p50 86, p90 485, p95 505, p99 511, truncation fallbacks 0, and longest inputs from `src/benchmark/benchmark.ts`, `src/graph/ladybug-store.ts`, `src/indexer/indexer.ts`, `src/indexer/relationship-graph.ts`, and `src/search/exact.ts`.

Planning-doc verification

- [x] Added the stable reference label `workstream:jina-tree-sitter-embedding-input-hardening` across the spec, plan, testing strategy, adversarial eval notes, design notes, and checkpoint checklist.
