  ...
  it("reuses unchanged embeddings, refreshes changed facts, and matches a fresh full index", async () => {
    ...
    try {
      ...
      try {
        ...
        const transitiveTotal = callees.results.find((result) => result.symbol?.name === "calculateGivingTotal");
        expect(calleeNames[0]).toBe("formatGivingReceipt");
        expect(calleeNames).toContain("formatGivingReceipt");
        expect(transitiveTotal?.metadata.relationship).toMatchObject({
          relationship: "transitive-call",
          evidenceSources: expect.arrayContaining(["transitive-call"]),
        });
      } finally {
        await engine.close();
      }
