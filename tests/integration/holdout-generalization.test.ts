  ...
  it("promotes package tsconfig aliases, lib source files, injected member methods, and route-string tests", async () => {
    ...
    try {
      ...
      try {
        ...
        const prismaReferences = await engine.getReferences("prisma", { limit: 20 });
        const linksRouteRank = prismaReferences.results.findIndex(
          (result) => result.file === "apps/web/app/api/links/route.ts",
        );
        expect(linksRouteRank).toBeGreaterThanOrEqual(0);
        expect(linksRouteRank + 1).toBeLessThanOrEqual(10);
