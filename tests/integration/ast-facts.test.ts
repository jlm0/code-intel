...
const fluentAndWrappedCallSource = `export async function runFlow(db: DbClient, tx: TxClient, gw: GatewayClient) {
  await db
    .insert(monitorRun)
    .values({ runnedAt: new Date() })
    .returning();

  await tx
    .delete(monitorTag)
    .where(eq(monitorTag.workspaceId, "workspace"))
    .run();

  await gw.request<ImageAttachResponse>("image.attach", { path: "/tmp/image.png" });

  void gw
    .request<{ path?: string }>("paste.collapse", { text: "large paste" })
    .then(() => undefined)
    .catch(() => undefined);
}
`;
...
describe("Tree-sitter structural facts", () => {
  it("extracts deterministic import, export, declaration, call, ownership, and test facts", async () => {
    ...
    expect(facts.testCases.map(testCaseShape)).toEqual([
      ...
      { kind: "Test", name: "it summarizes entries", parentName: "WidgetStore" },
    ]);
  ...
  it("normalizes fluent and wrapped member calls into stable member paths", async () => {
    const extractSourceFileFacts = await loadExtractor();

    const facts = extractSourceFileFacts({
      relativePath: "packages/server/src/run-flow.ts",
      content: fluentAndWrappedCallSource,
    });
    ...
    expect(facts.calls.map(callEvidenceShape)).toEqual(
      expect.arrayContaining([
        {
          callKind: "member",
          containingDeclarationName: "runFlow",
