...
describe("fusion module resolution", () => {
  it("resolves root tsconfig.base paths and star barrels to imported symbols", async () => {
    const repoPath = await mkdtemp(join(tmpdir(), "code-intel-fusion-barrel-repo-"));
    const indexPath = await mkdtemp(join(tmpdir(), "code-intel-fusion-barrel-index-"));
    try {
      await mkdir(join(repoPath, "src", "schema"), { recursive: true });
      await writeFile(join(repoPath, "package.json"), JSON.stringify({ name: "@fixture/barrel" }));
      await writeFile(join(repoPath, "tsconfig.json"), JSON.stringify({}));
      await writeFile(
        join(repoPath, "tsconfig.base.json"),
        JSON.stringify({
          compilerOptions: {
            baseUrl: ".",
            paths: {
      ...
      await writeFile(join(repoPath, "src", "schema", "index.ts"), `export * from "./monitor_run";\n`);
      await writeFile(
        join(repoPath, "src", "schema", "monitor_run.ts"),
        `export const monitorRun = {
  workspaceId: "workspace_id",
};
`,
      );
      await writeFile(
        join(repoPath, "src", "run.ts"),
        `import { monitorRun } from "@fixture/schema";
...
export function registerRunMonitor() {
  return db.insert(monitorRun.workspaceId);
}
`,
      );
      ...
      expect(resolutionFacts.repos[0]?.imports).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            importerFile: "src/run.ts",
            moduleSpecifier: "@fixture/schema",
            status: "resolved",
            resolutionSource: "typescript-paths",
            targetFile: "src/schema/index.ts",
            targetSymbolName: "monitorRun",
          }),
        ]),
      );
      ...
      try {
        ...
        const registerRunMonitor = nodes.find(
          (node) => node.kind === "Function" && node.name === "registerRunMonitor" && node.file === "src/run.ts",
        );
        const monitorRun = nodes.find(
          (node) => node.kind === "Symbol" && node.name === "monitorRun" && node.file === "src/schema/monitor_run.ts",
        );
        expect(registerRunMonitor).toBeDefined();
        expect(monitorRun).toBeDefined();
        expect(edges).toEqual(
          expect.arrayContaining([
            expect.objectContaining({
              kind: "REFERENCES",
              fromId: registerRunMonitor!.id,
              toId: monitorRun!.id,
