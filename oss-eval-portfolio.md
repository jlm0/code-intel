...
The 2026-05-17 holdout validation phase added Dub, Twenty, and Formbricks after that hardening pass. Their first corrected baseline intentionally ran before tuning implementation to the new repos and exposed fusion, graph, test-linking, and ranking gaps. The later holdout generalization hardening pass fixed the relationship target gaps without weakening the cases. All holdout packs still have zero required gates, so they are quality probes rather than blocking regression gates.

| Pack | Provider | Blocking | Quality | Target | Scoreboard | Failure Classes |
| --- | --- | --- | --- | ---: | ---: | --- |
| Dub | hash | pass | fail | 11/11 | 1/2 | ranking |
| Dub | Jina | pass | fail | 11/11 | 1/2 | ranking |
| Twenty | hash | pass | pass | 9/9 | 1/1 | none |
| Twenty | Jina | pass | pass | 9/9 | 1/1 | none |
| Formbricks | hash | pass | fail | 9/9 | 0/1 | ranking |
| Formbricks | Jina | pass | fail | 9/9 | 0/1 | ranking |

## Holdout Failure Classification

The original corrected holdout baseline failures were classified this way before the holdout generalization hardening pass:
...
After hardening, the remaining holdout failures are only scoreboard ranking probes:

| Pack | Failure | Classification | Affected Layer |
| --- | --- | --- | --- |
| Dub | `dub.query.webhook-payment-flow` still misses the expected webhook/payment flow rank target | ranking-only quality issue | ranking |
| Formbricks | `formbricks.query-survey-editor-auth-response-flow` still misses the expected survey editor auth/response flow rank target | ranking-only quality issue | ranking |

The relationship target gates now pass because the implementation added generic support for root `tsconfig.base.json` paths with blank root `tsconfig.json`, package-local compiler configs, first-party `lib/` source directories, imported member-call `CALLS` evidence, exact call-evidence edge variants, constructor-injected service method calls, Nest module/provider references, and HTTP harness route-string test links.
