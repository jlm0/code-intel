...
| Hermes Agent | hash | pass | fail | 5/9 | 0/1 | chunking, fusion, ranking, test-linking |
| Hermes Agent | Jina | pass | fail | 5/9 | 0/1 | chunking, fusion, ranking, test-linking |
...
The corrected hash and Jina baselines both pass blocking and fail quality with 5 of 9 target gates passing. The new composer helper graph target currently fails because the indexed graph does not resolve `ui-tui/src/__tests__/useComposerState.test.ts`, which keeps this as a real test discovery or test-linking hardening target instead of a misleading hook-reference expectation.
