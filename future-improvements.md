...
| Done | Test relationship linking | Better implementation-to-test retrieval | Direct, indirect, and exact colocated fallback `TESTS` edges now pass synthetic and Rallly test-linking gates with evidence and false-positive guards. |
| P1.4 | Eval corpus coverage diagnostics | Better eval trust | Some Rallly expectations may be missing because sparse checkout, discovery, or tsconfig filtering excluded files. |
...
The first Rallly run was the quality baseline after AST and SCIP hardening. It proved the eval harness worked, but also showed real app-flow retrieval was not good enough yet. The fusion, graph, ranking, and test-linking passes moved that baseline forward.
...
- Rallly target gates now pass 18 of 18 across hybrid ranking, graph traversal, full app-flow retrieval, and test-linking cases.
- Ranking-only target metrics after the ranking pass were `MRR@10 0.904762`, `Recall@K 0.880952`, and `nDCG@10 0.808548`. After adding test-linking gates, the full target aggregate is `MRR@10 0.809524`, `Recall@K 0.880952`, and `nDCG@10 0.763554`.
...
The AST/chunking foundation, SCIP-first relationship item, fusion resolution item, graph traversal item, deterministic hybrid ranking item, and test relationship linking item have now been completed. The next 80/20 order from the current evidence is:

1. Eval corpus coverage diagnostics.
...
2. Ranking generalization and optional reranker.
...
## Completed: Test Relationship Linking
...
- `TESTS` edges are now normalized from direct test imports, direct test calls, SCIP references, and concrete AST test cases.
- Bounded indirect links connect tests through implementation `CALLS`, `REFERENCES`, `IMPORTS`, and `EXPORTS` paths when direct evidence proves the starting test-to-implementation relationship.
- Exact colocated source/test naming fallback is available only when stronger evidence is absent and is labeled with `confidence: fallback` plus `fallbackReason`.
- CLI and MCP graph/query outputs expose test link kind, evidence sources, confidence, owner file, test case name, test case range, target symbol or file, fallback reason, and traversal path where relevant.
- Synthetic required eval gates pass direct call, indirect call, colocated fallback, and false-positive guard cases.
- Rallly target gates pass private route tests, route-to-mutation coverage, route-to-database coverage, middleware/API tests, implementation-to-test lookup, test-to-implementation lookup, and false-positive guards.

## P1.4: Eval Corpus Coverage Diagnostics
