...
| Done | Fusion module resolution and hybrid app-flow ranking | Highest JS/TS app-flow gain | Fusion now persists resolved module facts, resolves package exports and path aliases, writes evidence-backed import/export/reference/call/test edges, and hybrid ranking passes current Rallly target gates. |
| P1.0 | Graph traversal and path semantics | Highest explainability gain | New graph target gates intentionally fail for typed app-flow paths, middleware usage paths, UI-to-API paths, and test-to-implementation evidence. |
...
- Rallly target gates now pass 7 of 7 across hybrid ranking, graph traversal, and full app-flow retrieval cases.
...
The AST/chunking foundation, SCIP-first relationship item, fusion resolution item, and initial hybrid ranking item have now been completed. A stricter graph red-gate pass was added after fusion. The next 80/20 order from the current evidence is:

1. Graph traversal and path semantics.
   - The new Rallly target gates fail on route to mutation to database, middleware to route, UI to API, and test to implementation paths.
   - Fixing this should make graph answers explainable instead of only retrievable through semantic ranking.

2. Broader test relationship linking.
...
3. Eval corpus coverage diagnostics.
...
## P2: Query Ranking Follow-Up

Goal: refine ranking after P1 app-flow ranking, import/export graph, and SCIP-first evidence are in place.
...
- Add learned or configurable weights only after eval evidence shows static weights are insufficient.
- Tune ranking for query classes such as symbol lookup, semantic app-flow, test lookup, and context expansion.
- Use eval deltas to prevent overfitting to one Rallly flow.
- Keep ranking metadata visible for debugging.
- Add ranking metadata for explainability.

Acceptance criteria:

- Eval records ranking position.
- Expected results move toward the top after ranking changes.
- Ranking changes do not hide exact symbol matches.
