...
export function extractTestCaseFact(
  ...
  const callee = testCalleeForCall(node);
  if (!callee) {
    return undefined;
  }
  ...
  return {
    ...
    name: titleToTestName(callee, title),
    title,
    callee,
    parentName: nearestTestCaseTitle(ancestors)?.title,
  };
}
...
export function extractCallFact(input: ChunkSourceFileInput, node: TreeSitterNode): SourceCallFact | undefined {
  ...
  if (
    ...
    (node.type === "jsx_element" && children(node).some((child) => child.type === "jsx_opening_element" && child.text === "<>"))
  ) {
    return {
      ...factBase(input, node),
      name: "Fragment",
  ...
  const functionText = normalizeMemberPath(functionNode?.text ?? "");
  const name = extractCallableName(functionText);
  if (!name) {
    return undefined;
  }
  const parts = isMemberLikeCall(functionNode, functionText) ? memberParts(functionText) : undefined;
  const callKind = children(node).some((child) => child.type === "template_string")
    ? "tagged-template"
    : functionText === "import"
    ? "dynamic-import"
    : functionNode?.type === "super"
      ? "constructor"
    : parts
      ? "member"
      : "function";
  return {
    ...
    argumentSpecifier: moduleSpecifierForCallArgument(node),
  };
}

function isMemberLikeCall(functionNode: TreeSitterNode | null | undefined, normalizedText: string): boolean {
  if (!functionNode) {
    return false;
  }
  return (
    functionNode.type === "member_expression" ||
    functionNode.type === "await_expression" ||
    functionNode.type === "unary_expression" ||
    /^[A-Za-z_$][\w$]*(?:\?\.|\.)/.test(normalizedText)
  );
}
