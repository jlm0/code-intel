...
export function extractSourceFileFacts(input: ChunkSourceFileInput): SourceFileAstFacts {
  ...
  visitWithAncestors(tree.rootNode, [], (node, ancestors) => {
    ...
    if (declaration) {
      ...
      declarations.push(...compatibleDeclarationFacts(declaration));
      if (node.type === "enum_declaration") {
        declarations.push(...enumMemberDeclarations(input, declaration, node));
      }
    }
...
function compatibleDeclarationFacts(declaration: SourceDeclarationFact): SourceDeclarationFact[] {
  if (declaration.kind !== "VariableFunction" || !isFactoryCallDeclaration(declaration)) {
    return [];
  }
  return [{ ...declaration, kind: "Variable" }];
}

function isFactoryCallDeclaration(declaration: SourceDeclarationFact): boolean {
  return new RegExp(`^${escapeRegExp(declaration.name)}\\s*=\\s*[A-Za-z_$][\\w$]*(?:\\.[A-Za-z_$][\\w$]*)*\\s*\\(`)
    .test(declaration.sourceText.trim());
}
