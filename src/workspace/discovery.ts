...
export interface WorkspaceDiscoveryDiagnostics {
  files: DiscoveryFileDiagnostic[];
}

export interface DiscoveryFileDiagnostic {
  repo: string;
  absolutePath: string;
  relativePath: string;
  kind: "file" | "directory";
...
export async function discoverWorkspace(input: DiscoverWorkspaceInput): Promise<DiscoveredWorkspace> {
  ...
  const repos: DiscoveredRepo[] = discoveredRepos.map(({ discoveryDiagnostics: _diagnostics, ...repo }) => repo);
  ...
  return {
    ...
    repos,
    diagnostics: {
      files: discoveredRepos.flatMap((repo) => repo.discoveryDiagnostics),
    },
  };
}
...
): Promise<DiscoveredRepo & { discoveryDiagnostics: DiscoveryFileDiagnostic[] }> {
...
async function walk(
  ...
  onSkippedDirectory?: (path: string) => void,
): Promise<void> {
  for (const entry of await safeReaddir(root)) {
    ...
    if (entry.isDirectory()) {
      ...
      await walk(absolutePath, options, onFile, onSkippedDirectory);
