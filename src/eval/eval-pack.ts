...
export async function prepareEvalCorpus(input: PrepareEvalCorpusInput): Promise<PreparedEvalCorpus> {
  ...
  const checkoutPath = join(evalCachePath, pack.id, corpusCacheKey(pack.corpus));
...
function resolveEvalPackPath(input: ResolveEvalPackInput): string {
  ...
  if (!builtInSuiteIds.includes(suite)) {
...
function corpusCacheKey(corpus: Extract<EvalCorpus, { type: "git" }>): string {
  const sparseHash = createHash("sha256")
    .update(JSON.stringify({
      repoPaths: corpus.repoPaths,
      sparsePaths: corpus.sparsePaths,
      url: corpus.url,
    }))
    .digest("hex")
    .slice(0, 12);
  return `${corpus.ref.slice(0, 12)}-${sparseHash}`;
}
