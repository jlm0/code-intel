...
function codeAwareBoosts(query: QueryModel, candidate: HybridSemanticCandidate): number {
  ...
  if (candidate.paths?.length) boost += pathRankingBoost(candidate);
...
function appFlowBoost(
  ...
  if (query.words.includes("route") && candidate.node.kind === "File" && /\/routes?\/|\/api\/|\/route\.[cm]?[jt]sx?$/.test(path)) {
    boost += addReason(candidate, "app_flow_route_owner_file", 1.05);
  }
  if (query.words.includes("prisma") && candidate.paths?.length && candidate.node.kind === "File" && /\/routes?\/|\/api\/|\/route\.[cm]?[jt]sx?$/.test(path)) {
    boost += addReason(candidate, "app_flow_database_route_owner", 0.95);
  }
  if (
    query.words.includes("route") &&
    query.words.includes("handler") &&
    query.words.includes("wired") &&
    candidate.node.kind === "File" &&
    /\/routes?\/|\/api\/|\/route\.[cm]?[jt]sx?$/.test(path)
  ) {
    boost += addReason(candidate, "app_flow_route_wiring_file", 0.75);
  }
...
function codeAwareDemotions(query: QueryModel, candidate: HybridSemanticCandidate): number {
  ...
  if (!query.words.includes("schema") && /(schema|params|input)$/.test(name)) {
    demotion += addDemotion(candidate, "schema_helper", query.intent === "app-flow" ? 0.72 : 0.18);
  }
  ...
  if (
    query.intent === "app-flow" &&
    candidate.node.kind === "Symbol" &&
    !candidate.evidenceSources.has("mutation-to-database") &&
    !/prisma|database|db|client/.test(name)
  ) {
    demotion += addDemotion(candidate, "app_flow_demotes_unowned_value_symbol", 1.85);
  }
  return demotion;
}

function pathRankingBoost(candidate: HybridSemanticCandidate): number {
  const bestPath = [...(candidate.paths ?? [])].sort((left, right) => right.score - left.score)[0];
  if (!bestPath) {
    return 0;
  }
  return addReason(
    candidate,
    "path_app_flow",
    Math.min(2.1, Math.max(0.25, bestPath.score / 2.4)),
    bestPath.reason,
  );
}
...
function surfaceIntentBoost(
  ...
  const hasUiIntent = query.words.some((word) =>
    ["angular", "builder", "react", "component", "components", "page", "dashboard", "form", "table", "tui", "terminal", "hook", "ui"].includes(word)
  );
  if (hasUiIntent && isUiSurface(path, name)) {
    boost += addReason(candidate, "surface_ui", 0.46);
  }
  if (
    query.words.includes("builder") &&
    query.words.includes("link") &&
    /\/ui\/links?\//.test(path)
  ) {
    boost += addReason(candidate, "surface_link_builder", 1.65);
  }
  if (query.words.includes("builder") && /^use[A-Z]/.test(candidate.node.name ?? "")) {
    boost += addReason(candidate, "surface_builder_hook", 0.85);
  }
  ...
  if ((query.words.includes("page") || query.words.includes("dashboard") || query.words.includes("form") || query.words.includes("editor")) && isPageSymbol(candidate)) {
...
function isPageSymbol(candidate: HybridSemanticCandidate): boolean {
  return candidate.node.name === "Page" &&
    ...
    /(^|\/)(app|pages|modules)\/.*\/page\.[cm]?[jt]sx?$/.test(candidate.node.file ?? "");
}
