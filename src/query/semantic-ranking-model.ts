...
export function buildQueryModel(raw: string): QueryModel {
  ...
  return {
    ...
    words,
    codeTokens,
    symbolTokens: [...new Set([...rawCodeIdentifiers(raw), ...codeTokens, ...symbolWords(words)])].slice(0, 40),
  };
}
...
function semanticGraphTokens(query: string): string[] {
  ...
  if (words.includes("gateway")) {
    tokens.add("gatewayClient");
  }
  if (words.includes("hook")) {
    for (const word of words.filter((candidate) => candidate.length >= 4 && !["hook", "hooks"].includes(candidate))) {
      tokens.add(`use${capitalize(word)}`);
      tokens.add(`use${capitalize(word)}State`);
    }
  }
  return [...tokens].filter((token) => token === "app" || token.length >= 4).slice(0, 32);
}
