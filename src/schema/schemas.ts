...
export const IndexProgressCountersSchema = z.object({
  ...
  scipShardCost: z.number().min(0).optional(),
  scipShardFiles: z.number().int().min(0).optional(),
  ...
  physicalEdgesWritten: z.number().int().min(0).optional(),
...
export const IndexDiscoverySummarySchema = z.object({
  totals: z.object({
    ...
    unsupportedFiles: z.number().int().min(0),
      ...
      generatedFiles: z.number().int().min(0).default(0),
      buildArtifactFiles: z.number().int().min(0).default(0),
      generatedBytes: z.number().int().min(0).default(0),
      buildArtifactBytes: z.number().int().min(0).default(0),
    }),
  topExcludedContributors: z.array(
    z.object({
      path: z.string(),
      reason: z.string(),
