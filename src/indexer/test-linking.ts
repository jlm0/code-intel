...
function addHttpRouteTestEdges(input: ApplyTestLinkingGraphFactsInput): void {
  const routeTargets = httpRouteTargets(input);
  ...
  for (const [testFile, fileFact] of input.fileFactsByRelativePath) {
    ...
    for (const call of fileFact.calls) {
      const request = httpRequestForCall(call);
      if (!request) {
        continue;
      }
      for (const target of routeTargetsMatching(routeTargets, request.method, request.path)) {
        const testCase = testCaseForRange(fileFact, call.range)
          ?? (fileFact.testCases.length === 1 ? fileFact.testCases[0] : undefined);
        const metadata = httpRouteTestMetadata(input.repo.name, testFile, call, target, request, testCase);
        addEvidenceTestEdges(input, testFile, target.node, metadata);
      }
    }
  }
}
...
function httpRouteTargets(input: ApplyTestLinkingGraphFactsInput): HttpRouteTarget[] {
  const targets: HttpRouteTarget[] = [];
  for (const [relativePath, fileNode] of input.fileNodes) {
    if (fileNode.metadata.fileKind !== "source") {
      continue;
    }
    const path = routePathForFile(relativePath);
    if (!path) {
      continue;
    }
    const routeSymbols = routeHandlerSymbols(input, relativePath);
    if (routeSymbols.length === 0) {
      targets.push({ node: fileNode, file: relativePath, path });
      continue;
    }
    for (const symbol of routeSymbols) {
      targets.push({
        node: symbol,
        file: relativePath,
        method: httpMethodName(symbol.name),
        path,
      });
    }
  }
...
function routeHandlerSymbols(input: ApplyTestLinkingGraphFactsInput, relativePath: string): CodeNode[] {
  return (input.astSymbolsByFile.get(relativePath) ?? [])
    .filter((node) => node.metadata.fileKind === "source" && httpMethodName(node.name))
    .sort((left, right) => (left.name ?? "").localeCompare(right.name ?? "") || left.id.localeCompare(right.id));
}

function httpRequestForCall(call: FileFact["calls"][number]): HttpRouteRequest | undefined {
  const method = httpMethodName(call.propertyName ?? call.name);
  if (!method || call.callKind !== "member") {
    return undefined;
  }
  const path = httpPathFromCallSource(call.sourceText);
  return path ? { method, path } : undefined;
}

function httpPathFromCallSource(sourceText: string): string | undefined {
  const objectPath = sourceText.match(/\bpath\s*:\s*["'`]([^"'`]+)["'`]/)?.[1];
  const firstArgumentPath = sourceText.match(/\(\s*(?:<[^>]+>\s*)?["'`]([^"'`]+)["'`]/)?.[1];
  return normalizeHttpPath(objectPath ?? firstArgumentPath);
}

function routeTargetsMatching(
  targets: HttpRouteTarget[],
  method: string,
  requestPath: string,
): HttpRouteTarget[] {
  return targets.filter((target) =>
    (!target.method || target.method === method) &&
    routePathMatches(target.path, requestPath)
  );
}

function routePathForFile(relativePath: string): string | undefined {
  const appRouteMatch = relativePath.match(/(?:^|\/)app\/api\/(.+)\/route\.[cm]?[jt]sx?$/);
  if (appRouteMatch?.[1]) {
    return normalizeRoutePath(appRouteMatch[1]);
  }
  const pagesRouteMatch = relativePath.match(/(?:^|\/)pages\/api\/(.+)\.[cm]?[jt]sx?$/);
  if (pagesRouteMatch?.[1]) {
    return normalizeRoutePath(pagesRouteMatch[1]);
  }
  return undefined;
}
...
function routePathMatches(routePath: string, requestPath: string): boolean {
  const normalizedRoutePath = normalizeHttpPath(routePath);
  const normalizedRequestPath = normalizeHttpPath(requestPath);
  if (!normalizedRoutePath || !normalizedRequestPath) {
    return false;
  }
  const candidates = new Set([
    normalizedRoutePath,
    normalizedRoutePath.startsWith("/api/") ? normalizedRoutePath.slice(4) : `/api${normalizedRoutePath}`,
  ]);
  return [...candidates].some((candidate) => pathSegmentsMatch(candidate, normalizedRequestPath));
}

function pathSegmentsMatch(routePath: string, requestPath: string): boolean {
  const routeSegments = routePath.split("/").filter(Boolean);
  const requestSegments = requestPath.split("/").filter(Boolean);
  if (routeSegments.length !== requestSegments.length) {
    return false;
  }
  return routeSegments.every((segment, index) => segment.startsWith(":") || segment === requestSegments[index]);
}
...
function httpMethodName(value: string | undefined): string | undefined {
  const method = value?.toUpperCase();
  return method && ["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"].includes(method)
    ? method
    : undefined;
}
...
function directTestMetadata(
  ...
  const linkKind = stringFromMetadata(metadata, "testLinkKind") ?? (metadataArrayIncludes(metadata.roles, "Call")
    ? "direct-call"
...
function httpRouteTestMetadata(
  repo: string,
  testFile: string,
  call: FileFact["calls"][number],
  ...
  return addTestCaseMetadata({
    ...
    ownerFile: testFile,
    testOwnerFile: testFile,
    origin: "test-linking-http-route",
    source: "test-linking",
    testContext: true,
    testLinkKind: "http-route",
    confidence: "medium",
    evidenceSources: ["http-harness-route", "tree-sitter-test"],
    roles: ["Test", "Call"],
    range: call.range,
    ...
    targetSymbolId: target.node.id,
    targetSymbolName: target.node.name,
  }, testCase);
}
