...
interface ResolutionContext extends BuildRepoResolvedModuleFactsInput {
  ...
  tsPaths: string[];
  compilerConfigs: CompilerConfigResolution[];
}

interface CompilerConfigResolution {
  directory: string;
  compilerOptions: ts.CompilerOptions;
  tsPaths: string[];
}
...
async function createResolutionContext(input: BuildRepoResolvedModuleFactsInput): Promise<ResolutionContext> {
  return {
    ...
    ...(await loadCompilerResolution(input.repo)),
  };
}
...
function resolveWithTypescript(
  ...
  const compilerOptions = compilerOptionsForImporter(context, importerFile);
  ...
  return resolvedTarget(context, targetFile, resolutionSourceForTs(context, importerFile, moduleSpecifier));
}
...
async function loadCompilerResolution(repo: DiscoveredRepo): Promise<{
  compilerOptions: ts.CompilerOptions;
  tsPaths: string[];
  compilerConfigs: CompilerConfigResolution[];
}> {
  ...
  if (!configPath) {
    const fallback = defaultCompilerResolution(repoPath);
    return { ...fallback, compilerConfigs: [] };
  }
  ...
  const packageConfigs = packageCompilerConfigs(repo, compilerOptions, Object.keys(compilerOptions.paths ?? {}));
  return {
    compilerOptions,
    tsPaths: Object.keys(compilerOptions.paths ?? {}),
    compilerConfigs: packageConfigs,
  };
}

function packageCompilerConfigs(
  repo: DiscoveredRepo,
  defaultCompilerOptions: ts.CompilerOptions,
  defaultTsPaths: string[],
): CompilerConfigResolution[] {
  const configs = new Map<string, CompilerConfigResolution>();
  configs.set(repo.path, {
    directory: repo.path,
    compilerOptions: defaultCompilerOptions,
    tsPaths: defaultTsPaths,
  });

  for (const pkg of repo.packages) {
    if (pkg.path === repo.path) {
      continue;
    }
    const configPath = directConfigPath(pkg.path);
    if (!configPath) {
      continue;
    }
    const parsed = parseCompilerConfig(configPath);
    ...
    const compilerOptions = compilerOptionsForResolution(parsed.options, dirname(configPath));
    configs.set(pkg.path, {
      directory: pkg.path,
      compilerOptions,
      tsPaths: Object.keys(compilerOptions.paths ?? {}),
    });
  ...
  return [...configs.values()].sort((left, right) => right.directory.length - left.directory.length);
}

function directConfigPath(directory: string): string | undefined {
...
function compilerOptionsForImporter(context: ResolutionContext, importerFile: string): ts.CompilerOptions {
  const importerPath = resolve(context.repo.path, importerFile);
  return compilerConfigForPath(context, importerPath)?.compilerOptions ?? context.compilerOptions;
}

function compilerConfigForPath(context: ResolutionContext, absolutePath: string): CompilerConfigResolution | undefined {
  return context.compilerConfigs.find((config) =>
    absolutePath === config.directory || absolutePath.startsWith(`${config.directory}/`)
  );
}

function resolutionSourceForTs(context: ResolutionContext, importerFile: string, moduleSpecifier: string): string {
  if (moduleSpecifier.startsWith(".")) {
    return "typescript-relative";
  }
  const importerPath = resolve(context.repo.path, importerFile);
  const tsPaths = compilerConfigForPath(context, importerPath)?.tsPaths ?? context.tsPaths;
  return tsPaths.some((pattern) => pathPatternMatches(pattern, moduleSpecifier))
    ? "typescript-paths"
    : "typescript";
}
