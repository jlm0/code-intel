  ...
  for (const [relativePath, fileFact] of input.fileFactsByRelativePath) {
    for (const call of fileFact.calls) {
      ...
      for (const source of sourceSymbols) {
        ...
        const injectedTarget = injectedMemberCallTarget(symbols, relativePath, fileFact, call);
        if (injectedTarget && source.id !== injectedTarget.id) {
          input.addEdge(
            "CALLS",
            source.id,
            injectedTarget.id,
...
function injectedMemberCallTarget(
  symbols: CodeNode[],
  relativePath: string,
  fileFact: FileFact,
  call: FileFact["calls"][number],
  ...
  return injectedTypeName ? findSymbolByName(symbols, injectedTypeName, relativePath) : undefined;
}
