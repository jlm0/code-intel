...
export function applyScipGraphFacts(input: ScipFusionInput): ScipFusionResult {
  ...
  for (const reference of input.facts.references) {
    ...
    const containingSymbol = findContainingSymbol(input.astSymbolsByFile.get(reference.relativePath) ?? [], reference.range);
    const typeRelationship = fileFact
      ? typeRelationshipForReference(fileFact, reference, symbolNode)
      : undefined;
    if (containingSymbol && containingSymbol.id !== symbolNode.id && typeRelationship) {
      const typeMetadata = typeRelationshipMetadata(referenceMetadata, reference, typeRelationship);
      input.addEdge("REFERENCES", containingSymbol.id, symbolNode.id, input.workspaceName, input.repo.name, typeMetadata);
      if (typeRelationship === "extends") {
        input.addEdge("EXTENDS", containingSymbol.id, symbolNode.id, input.workspaceName, input.repo.name, typeMetadata);
      }
      if (typeRelationship === "implements") {
        input.addEdge("IMPLEMENTS", containingSymbol.id, symbolNode.id, input.workspaceName, input.repo.name, typeMetadata);
      }
    }
    if (!containingChunk) {
      continue;
...
function typeRelationshipForReference(
  fileFact: FileFact,
  reference: ScipReference,
  target: CodeNode,
): "extends" | "implements" | "type-use" | undefined {
  if (!isTypeLikeReference(reference, target)) {
    return undefined;
  }
  const declaration = findContainingDeclaration(fileFact.declarations, reference.range);
  if (!declaration) {
    return undefined;
  }
  if (declaration.kind === "Class" && heritageClauseContains(declaration.sourceText, "extends", reference.symbolName)) {
    return "extends";
  }
  if (declaration.kind === "Class" && heritageClauseContains(declaration.sourceText, "implements", reference.symbolName)) {
    return "implements";
  }
  return "type-use";
}

function isTypeLikeReference(reference: ScipReference, target: CodeNode): boolean {
  return ["Class", "Interface", "TypeAlias"].includes(target.kind) ||
    ["Class", "Interface", "TypeAlias", "TypeParameter"].includes(reference.symbolKind);
}

function heritageClauseContains(sourceText: string, keyword: "extends" | "implements", symbolName: string): boolean {
  const escapedName = symbolName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  if (keyword === "extends") {
    return new RegExp(`\\bextends\\s+${escapedName}\\b`).test(sourceText);
  }
  const implementsMatch = sourceText.match(/\bimplements\s+([^{]+)/);
  return Boolean(implementsMatch?.[1] && new RegExp(`\\b${escapedName}\\b`).test(implementsMatch[1]));
}

function typeRelationshipMetadata(
  base: Record<string, unknown>,
  reference: ScipReference,
  relationship: "extends" | "implements" | "type-use",
  ...
  return {
    ...
    evidenceSources: mergeStringArrays(base.evidenceSources, relationship === "type-use" ? "type-use" : "type-relationship"),
    confidence: "high",
    roles: mergeStringArrays(base.roles, "TypeUse"),
  };
}

function findContainingDeclaration(
  declarations: FileFact["declarations"],
  range: ScipRange,
): FileFact["declarations"][number] | undefined {
  return declarations
    .filter((declaration) => rangeContains(declaration.range, range))
    .sort((left, right) => rangeSize(left.range) - rangeSize(right.range))[0];
}

function findContainingSymbol(symbols: CodeNode[], range: ScipRange): CodeNode | undefined {
  return symbols
    .filter((symbol) => symbol.range && rangeContains(symbol.range, range))
    .sort((left, right) => rangeSize(left.range) - rangeSize(right.range))[0];
}
